...
int coap_make_response(coap_rw_buffer_t *scratch, coap_packet_t *pkt, const uint8_t *content, size_t content_len, uint8_t msgid_hi, uint8_t msgid_lo, const coap_buffer_t* tok, coap_responsecode_t rspcode, coap_content_type_t content_type)
{
    ...
    pkt->numopts = 0; // Start with 0 options
    ...
    // Only add content format option if a payload is present
    if (content_len > 0)
    {
        ...
        // Handle TEXT_PLAIN (0) correctly
        if (content_type == COAP_CONTENTTYPE_TEXT_PLAIN) {
            pkt->opts[0].buf.len = 0; // Length is 0 for content type 0
        } else {
             // Use 2 bytes for other types (like 40)
...
// -------------------------------------------------------------------
// HELPER FUNCTIONS ADDED FOR OBSERVE AND BLOCK2
// -------------------------------------------------------------------

/**
 * @brief Adds an option to a CoAP packet.
 * @param pkt   The packet to add the option to.
 * @param num   The option number.
 * @param buf   The option payload.
 * @param len   The length of the option payload.
 * @return 0 on success, or a CoAP error code.
 */
int coap_add_option(coap_packet_t *pkt, uint8_t num, const uint8_t *buf, size_t len)
{
    ...
    if (pkt->numopts > 0 && pkt->opts[pkt->numopts - 1].num > num) {
        // This simple implementation requires options to be added in order
        return COAP_ERR_UNSUPPORTED; 
    }

    pkt->opts[pkt->numopts].num = num;
    pkt->opts[pkt->numopts].buf.p = buf;
    pkt->opts[pkt->numopts].buf.len = len;
...
/**
 * @brief Encodes an integer value into a CoAP option buffer.
 * @param buf   The buffer to write the encoded integer to.
 * @param val   The integer value to encode.
 * @return The number of bytes written to the buffer.
 */
size_t coap_set_option_uint(uint8_t *buf, uint32_t val)
{
    if (val == 0) {
        return 0;
    } else if (val < 256) {
...
/**
 * @brief Decodes a CoAP option buffer into an integer.
 * @param buf   The option buffer to read from.
 * @return The decoded integer value.
 */
uint32_t coap_get_option_uint(const coap_buffer_t *buf)
{
    if (buf->len == 0) {
        return 0;
    } else if (buf->len == 1) {
        ...
        return (buf->p[0] << 8) | buf->p[1];
    } else {
        return (buf->p[0] << 16) | (buf->p[1] << 8) | buf->p[2];
    }
}